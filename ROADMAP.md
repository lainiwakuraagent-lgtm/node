...
| check_replies.sh | Confirmed genuinely dead, not just unwired: it and `telegram_webhook_handler.py` are a superseded webhook-based delivery generation, predating the current persistent long-poll design (`telegram_watcher.py` + `conversation.sh`). Mechanically unable to function even if manually started — `conversation.sh` actively deletes any registered webhook on every startup (a precondition for `getUpdates` long-polling, which is mutually exclusive with webhook delivery on Telegram's API), so nothing would ever call the webhook handler regardless. `telegram_check.sh` is the same generation's polling fallback. Deliberate decision: superseded, not a wiring gap — candidates for deletion, not integration, whenever someone wants to do that cleanup. | Resolved (confirmed dead) 2026-08-04 |
| conv_idle_check.py | Fixed 2026-08-04 — `telegram_watcher.py` now writes `state/conversation/last_real_message_at.txt` on every genuine message, so the 30-min idle-close signal can actually fire. Was previously dependent on a file nothing wrote. | Resolved 2026-08-04 |
